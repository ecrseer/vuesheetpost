...
export default {
  ...
  methods: {
    salvar() {
      ...
      if (this.editando) {
        axios.put(
            `https://sheetdb.io/api/v1/${import.}/id/${this.time.id}`,
            {data: [this.time]}
        ).then(() => {
        ...
        axios.post(
            `https://sheetdb.io/api/v1/${import.}`,
            {data: [this.time]}
        ).then(() => {
    ...
    apagar(time, index) {
      this.carregando = true
      axios.delete(`https://sheetdb.io/api/v1/${import.}/id/${time.id}`).then(() => {
  ...
  mounted() {
    this.carregando = true
    axios.get(`https://sheetdb.io/api/v1/${import.}`).then(({data}) => {
      this.times = data
      this.carregando = false
    })
  }
